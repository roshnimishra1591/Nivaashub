...
import {
  FaFileUpload, FaTimes, FaPlusCircle, FaMapMarkerAlt, FaDollarSign,
  FaHome, FaBed, FaSpinner, FaCheckCircle, FaExclamationCircle,
...
export default function OwnerAddProperty() {
  ...
  const [description, setDescription] = useState(""); // Renamed 'desc' to 'description' for clarity
  const [type, setType] = useState("Apartment");
  const [bedrooms, setBedrooms] = useState(1);
  const [bathrooms, setBathrooms] = useState(1); // Added bathrooms
  const [area, setArea] = useState(""); // Added area (sq ft)
  const [amenities, setAmenities] = useState([]); // For checkboxes, or multi-select
  ...
  const fileInputRef = useRef(null); // Ref for file input for programmatic click

  // List of common amenities (example)
  ...
  const handleDragOver = (e) => {
    e.preventDefault();
    e.stopPropagation();
    e.currentTarget.classList.add('border-blue-500', 'bg-blue-50'); // Add visual cue
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("description", description); // Use 'description'
      formData.append("type", type);
      formData.append("bedrooms", bedrooms);
      formData.append("bathrooms", bathrooms); // Append bathrooms
      formData.append("area", area); // Append area
      formData.append("amenities", JSON.stringify(amenities)); // Send amenities as a JSON string

      images.forEach((image) => {
        formData.append("images", image); // backend must accept multiple "images"
      });

      // Dummy API call URL. Replace with your actual backend endpoint.
      // For Rajbiraj, Nepal context, ensure your API endpoint is accessible.
      ...
      <div className="flex justify-center p-4 sm:p-6 lg:p-8">
        <div className="bg-white rounded-xl shadow-2xl p-8 md:p-10 lg:p-12 w-full max-w-4xl border border-blue-100 animate-fade-in-up">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-blue-900 mb-6 text-center">
            <FaPlusCircle className="inline-block mr-3 text-blue-600" /> List a New Property
          </h2>
          <p className="text-center text-gray-700 mb-10 text-lg leading-relaxed max-w-2xl mx-auto">
            Provide details about your rental space. High-quality information and photos attract more tenants!
          </p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {imagePreviews.map((img, index) => (
                    <div key={index} className="relative aspect-video rounded-lg overflow-hidden shadow-md group">
                      ...
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); removeImage(index); }} // Prevent parent click on remove
