...
import {
  FaUserFriends, FaSpinner, FaExclamationCircle, FaArrowLeft,
...
export default function AdminTenants() {
  ...
  const [actionLoadingId, setActionLoadingId] = useState(null); // To track loading for individual tenant actions
  ...
  const fetchTenants = async () => {
    ...
    try {
      ...
      let params = {
        status: filterStatus === 'all' || filterStatus === 'member' || filterStatus === 'nonmember' ? undefined : filterStatus,
      ...
      if (filterStatus === 'member') {
        tenants = tenants.filter(t => t.isMember);
      } else if (filterStatus === 'nonmember') {
        tenants = tenants.filter(t => !t.isMember);
      }
      // Sort: members first, then non-members
      tenants = tenants.sort((a, b) => (b.isMember === true) - (a.isMember === true));
      setTenants(tenants);
    } catch (err) {
      console.error("Failed to fetch tenants:", err.response?.data || err.message);
  ...
  const handleTenantAction = async (tenantId, actionType, value = null) => {
    if (actionType === 'delete') {
      if (!window.confirm("Are you sure you want to delete this tenant and all their associated data? This action cannot be undone.")) {
        return;
      }
    } else if (actionType === 'toggleStatus') {
        if (!window.confirm(`Are you sure you want to change this tenant's status to ${value ? 'Active' : 'Inactive'}?`)) {
            return;
        }
    }
    ...
    try {
      ...
      switch (actionType) {
        case 'delete':
          endpoint = `${API_URL}/admin/tenants/${tenantId}`;
          method = 'delete';
          break;
        case 'toggleStatus':
          endpoint = `${API_URL}/admin/tenants/${tenantId}/status`;
          method = 'put';
          data = { isActive: value };
          break;
        default:
      ...
      console.error(`Failed to ${actionType} tenant:`, err.response?.data || err.message);
      setError(`Failed to ${actionType} tenant. Please try again.`);
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100">
        ...
        <p className="text-xl text-gray-700 font-semibold">Loading all tenants...</p>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-xl shadow-2xl p-8 md:p-10 lg:p-12 w-full max-w-7xl border border-blue-100 animate-fade-in-up">
          <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-blue-900 flex items-center gap-3">
              <FaUserFriends className="text-blue-600" /> All Tenants
            </h2>
            {/* Optionally add a button to invite new tenants or add manually */}
            {/* <button
              onClick={() => alert("Invite New Tenant functionality")}
          ...
          <div className="mb-8 flex flex-col sm:flex-row gap-4 items-center justify-between p-4 bg-gray-50 rounded-lg shadow-inner">
            ...
            {/* Remove the status filter dropdown */}
            <div className="w-full sm:w-auto">
              <label htmlFor="filterMembership" className="sr-only">Filter by Membership</label>
              <select
                id="filterMembership"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white mb-2"
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value)}
              >
                <option value="all">All</option>
                <option value="member">Membership Taken</option>
                <option value="nonmember">Not Membered</option>
          ...
          {tenants.length === 0 ? (
            <div className="text-center py-16">
              <FaUserFriends className="text-blue-400 text-6xl mx-auto mb-6" />
              <p className="text-xl text-gray-700 font-semibold mb-4">No tenants found matching your criteria.</p>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {tenants.map((tenant) => (
                    <tr key={tenant._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-10 w-10">
                            <img className="h-10 w-10 rounded-full object-cover" src={tenant.profileImage || "/default-avatar.png"} alt={tenant.name} />
                          </div>
                          <div className="ml-4">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 flex items-center gap-1"><FaPhone className="text-gray-400 text-xs" /> {tenant.phone || 'N/A'}</div>
                        <div className="text-sm text-gray-500 flex items-center gap-1"><FaEnvelope className="text-gray-400 text-xs" /> {tenant.email || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center gap-1">
                          <FaCalendarAlt className="text-gray-400 text-xs" />
                          {tenant.createdAt ? new Date(tenant.createdAt).toLocaleDateString('en-NP', { year: 'numeric', month: 'short', day: 'numeric' }) : 'N/A'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex flex-wrap justify-center gap-2">
                          <Link to={`/admin/tenant-details/${tenant._id}`} className="text-blue-600 hover:text-blue-900 px-2 py-1 rounded-md bg-blue-50 hover:bg-blue-100 transition-colors flex items-center gap-1">
                            <FaEye /> View
                          </Link>
                          <button
                            onClick={() => handleTenantAction(tenant._id, 'toggleStatus', !tenant.isActive)}
                            disabled={actionLoadingId === tenant._id}
                            className={`px-2 py-1 rounded-md font-semibold transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed ${tenant.isActive ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}
                          >
                            {actionLoadingId === tenant._id ? <FaSpinner className="animate-spin text-sm" /> : (tenant.isActive ? <FaToggleOff /> : <FaToggleOn />)} {tenant.isActive ? 'Deactivate' : 'Activate'}
                          </button>
                          <button
                            onClick={() => handleTenantAction(tenant._id, 'delete')}
                            disabled={actionLoadingId === tenant._id}
                          ...
                          >
                            {actionLoadingId === tenant._id ? <FaSpinner className="animate-spin text-sm" /> : <FaTrash />} Delete
                          </button>
                        </div>
                      </td>
                    </tr>
